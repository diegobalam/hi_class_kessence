...
//           }
	...
	if (has_dxdy_guess_smg == _FALSE_){
     ...
     if (strcmp(string1,"k_essence2") == 0) {
	pba->gravity_model_smg = k_essence2;
	pba->field_evolution_smg = _TRUE_;
    pba->is_quintessence_smg = _TRUE_;
	flag2=_TRUE_;
    ...
    double phi_prime_ini_smg = pba->parameters_smg[3];
    double phi_ini_smg =  pba->parameters_smg[4];
	...
	if (has_dxdy_guess_smg == _FALSE_){
	  ...
	  if(pba->tuning_index_smg == 1){
	    ...
	    V0 = pba->Omega0_smg/(1+exp(-lambda*phi_end_guess));
            pba->tuning_dxdy_guess_smg = 1/(1+exp(-lambda*phi_end_guess));
            pba->parameters_smg[1] = V0;
	    
	  }

	  if(pba->tuning_index_smg == 4){
            phi_ini_smg = -log(pba->Omega0_smg/V0-1)/lambda;
            pba->parameters_smg[4] = phi_ini_smg;
            pba->tuning_dxdy_guess_smg = -exp(lambda*phi_ini_smg)/(V0*lambda);
	  }
	}//end of no has_dxdy_guess_smg
      }//end of quintessence_exp
...
if (strcmp(string1,"nkgb") == 0 || strcmp(string1,"n-kgb") == 0 || strcmp(string1,"N-KGB") == 0 || strcmp(string1,"nKGB") == 0) {
  ...
  class_test(pba->parameters_smg[2]<0.,errmsg,"In n-KGB, Rshift0>=0, or ICs for background can't be set.");
      ...
      class_test(flag2==_FALSE_,
		 errmsg,
		 "could not identify gravity_theory value, check that it is one of 'propto_omega', 'propto_scale', 'constant_alphas', 'eft_alphas_power_law', 'eft_gammas_power_law', 'eft_gammas_exponential', 'brans_dicke', 'galileon', 'nKGB', 'quintessence_exp','quintessence_monomial', 'quintessence_tracker', 'alpha_attractor_canonical' ...");
