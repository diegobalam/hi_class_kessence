...
int background_gravity_functions(
  ...
  if (pba->field_evolution_smg == _TRUE_) {
    ...
    if (pba->gravity_model_smg == quintessence_monomial) {
     ...
     else if (pba->gravity_model_smg == k_essence2) {
      
      double lambda=pba->parameters_smg[0];
      double V0 = pba->parameters_smg[1];
      double c=pba->parameters_smg[2];
      double V=V0*(1+exp(-lambda*phi));
      double Vprime=-V0*lambda*exp(-lambda*phi);
            
      G2 = X - pow(pba->H0/pba->h,2.)*V0*(1+exp(-lambda*phi))-2*pow(pba->H0/pba->h,2.)*X*c*Vprime; // V written as in arXiv:1406.2301 in CLASS units
      G2_X = 1-2*c*Vprime;
      G2_phi =pow(pba->H0/pba->h,2.)*Vprime-pow(lambda,2)*exp(-lambda*phi);
    ...
    class_test_except((A*M - B*F) == 0 ,
	       pba->error_message,
         free(pvecback);free(pvecback_B),
               "scalar field mixing with metric has degenerate denominator at a = %e, phi = %e, phi_prime = %e \n with A = %e, M =%e, B=%e, F=%e, \n H=%e, E0=%e, E1=%e, E2=%e, E3=%e \n M_*^2 = %e Kineticity = %e, Braiding = %e,G4= %e,G4_X= %e,G5_phi= %e,G4_XX= %e, G5_Xphi= %e,X= %e,G2=%e,G2_X=%e,G3_phi=%e,Omega0=%e,rho_to=%e,lambda=%e,V0=%e,phiprime=%e,phi=%e",
	       a,phi,phi_prime, A, M, B, F,
	       pvecback[pba->index_bg_H],E0,E1,E2,E3,
	       pvecback[pba->index_bg_M2_smg], pvecback[pba->index_bg_kineticity_smg],pvecback[pba->index_bg_braiding_smg],G4,G4_X,G5_phi,G4_XX,G5_Xphi,X,G2,G2_X,G3_phi,pba->Omega0_smg,rho_tot,pba->parameters_smg[0],pba->parameters_smg[1],pba->parameters_smg[2],pba->parameters_smg[3]);
